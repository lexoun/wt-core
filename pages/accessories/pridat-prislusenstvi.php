...
if (isset($_REQUEST['action']) && $_REQUEST['action'] == "add") {
    ...
    if (isset($_POST['variantbox']) && $_POST['variantbox'] == '1' && !empty($post)) {
    ...
    $insert = $mysqli->query("INSERT INTO products (spare_part, manufacturer, delivery_time, main_warehouse, up_selling, cross_selling, height, weight, width, length, type, productname, short_description, description, seourl, availability, ean, code, purchase_price, wholesale_price, price)
	VALUES ('" . $_POST['spare_part'] . "', '" . $_POST['manufacturer'] . "', '" . $_POST['delivery_time'] . "', '" . $_POST['main_warehouse'] . "', '$up_selling', '$cross_selling', '$height', '$weight', '$width', '$length', '$type', '$product_name', '" . $_POST['short_description'] . "', '" . $_POST['description'] . "',  '$seoslug', '" . $_POST['availability'] . "', '$ean_main', '$code', '$purchase_price', '$wholesale_price', '" . $price . "')") or die($mysqli->error);
...
//URL PRO ESHOPY
    ...
    if (!empty($post)) {
        ...
        foreach ($post as $post_index => $posterino) {
            ...
            if (!empty($posterino['variation_sku'])) {

                $variation_price = preg_replace("/[^0-9]/", "", $posterino['variation_price']);

                $variation_purchase_price = preg_replace("/[^0-9]/", "", $posterino['variation_purchase_price']);

                $variation_wholesale_price = preg_replace("/[^0-9]/", "", $posterino['variation_wholesale_price']);

                $variation_sale_price = preg_replace("/[^0-9]/", "", $posterino['variation_sale_price']);

                $ean = preg_replace('/\s+/', '', $posterino['variation_ean']);
                $sku = preg_replace('/\s+/', '', $posterino['variation_sku']);

                $insert = $mysqli->query("INSERT INTO products_variations (main_warehouse, purchase_price, wholesale_price, price, weight, length, width, height, product_id, sale_price, description, stock, ean, sku)
			  	VALUES ('" . $posterino['variation_main_warehouse'] . "','$variation_purchase_price', '$variation_wholesale_price', '$variation_price', '" . $posterino['variation_weight'] . "', '" . $posterino['variation_length'] . "', '" . $posterino['variation_width'] . "', '" . $posterino['variation_height'] . "', '$id','$variation_sale_price','" . $posterino['variation_description'] . "','" . $posterino['variation_stock'] . "','$ean','$sku')") or die($mysqli->error);
                ...
                while ($shop = mysqli_fetch_array($shops_query)) {
                    ...
                    if ($_POST[$postname] == '1') {
                        ...
                        $this_price = 0;
                        $this_price = preg_replace("/[^0-9]/", "", $posterino['variation_price']);

                        $sale_price = $_POST[$slug . '_sale_price'];

                        $insert = $mysqli->query("INSERT INTO products_variations_sites (product_id, variation_id, site) VALUES ('" . $id . "', '" . $variation_id . "', '$slug')") or die($mysqli->error);
                ...
                foreach ($posterino['variation_name'] as $index => $variation_name) {

                    $variation_value = $mysqli->real_escape_string($posterino['variation_value'][$index]);

                    $upperName = ucfirst($variation_name);
                    $upperValue = ucfirst($variation_value);

                    $insert = $mysqli->query("INSERT INTO products_variations_values (variation_id, name, value)
			  	VALUES ('$variation_id', '$upperName', '$upperValue')") or die($mysqli->error);

                }

                if ($_FILES['variation_picture']['size'][$post_index] != 0 && $_FILES['variation_picture']['error'][$post_index] == 0) {
                ...
                $locations_query = $mysqli->query("SELECT * FROM shops_locations");
                while ($location = mysqli_fetch_array($locations_query)) {

                    $location_id = $location['id'];
                    $instock = preg_replace('/\s+/', '', $posterino['new_stock_' . $location_id]);

                    $mysqli->query("INSERT INTO products_stocks (product_id, variation_id, instock, location_id) VALUES ('$id', '$variation_id', '$instock','" . $location['id'] . "')") or die($mysqli->error);
                ...
                <div style="display: inline-block; width: 100%; margin-top: 14px;">
                    <div class="form-group form-label-group" style="float: left; width: 50%; margin: 0; padding: 0 !important;">
                        ...
                        <div class="col-sm-6">
                            ...
                            <?php $manufactures_query = $mysqli->query("SELECT * FROM products_manufacturers 
                                WHERE type = 'manufacturer' ORDER BY manufacturer") or die($mysqli->error);?>
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <div class="panel panel-primary" data-collapsed="0">
                ...
                <div class="panel-body">
                    <div class="form-group">
                        <div class="col-sm-12">
                            <ul class="icheck-list">
                                <li>
                                    <input tabindex="5" value="1" name="variantbox" type="checkbox" id="variantsbox" class="icheck-16">
                                    <label for="minimal-checkbox-1-15">Produkt s variantami</label>
                                </li>
                            </ul>
                        </div>
                    </div>
                ...
                <div class="form-group variants" style="display: none;">
