...
$pagetitle = "Historie vlastní spotřeby";
...
<div class="row">
	...
	<div class="col-md-4 col-sm-5">
		...
		<a href="vlastni-spotreba" style=" margin-right: 24px; float: right;" class="btn btn-default btn-icon icon-left btn-lg">
					<i class="entypo-plus"></i>
					Vyskladnit do vlastní spotřeby
				</a>
...
<br>
	...
	<?php
$consumption_max_query = $mysqli->query('SELECT COUNT(*) AS NumberOfOrders FROM consumption') or die($mysqli->error);
$consumption_max = mysqli_fetch_array($consumption_max_query);
$max = $consumption_max['NumberOfOrders'];
...
$consumption_query = $mysqli->query("SELECT *, DATE_FORMAT(order_date, '%d. %m. %Y') as recieved_date FROM consumption ORDER BY id DESC limit " . $s_pocet . "," . $perpage) or die($mysqli->error);

?>


<?php
if (mysqli_num_rows($consumption_query) > 0) { ?>

<table class="table table-bordered table-striped datatable dataTable">
	<thead> <tr> <th>ID</th>
		<th style="width: 1000px;">Spotřebované zboží</th> <th>Datum spotřeby</th><th>Pobočka</th> <th class="text-center">Nákupní cena</th></tr> </thead>

	<tbody role="alert" aria-live="polite" aria-relevant="all">
<?php
    while ($consumption = mysqli_fetch_array($consumption_query)) {

        consumption($consumption, $client['secretstring'], 0);
