...
if (isset($_REQUEST['action']) && $_REQUEST['action'] == "storno_invoice") {

    $invoice_id = intval($_REQUEST['id']);

    $invoice_check = $mysqli->query("SELECT id, storno FROM demands_advance_invoices WHERE id = '" . $invoice_id . "'") or die($mysqli->error);
    $invoice_data = mysqli_fetch_assoc($invoice_check);

    if ($invoice_data) {
        // Přepínáme storno stav (pokud je stornováno, zrušíme storno a naopak)
        $new_storno = ($invoice_data['storno'] == 1) ? 0 : 1;
        $mysqli->query("UPDATE demands_advance_invoices SET storno = '" . $new_storno . "' WHERE id = '" . $invoice_id . "'") or die($mysqli->error);
    }

    // Zachováme všechny původní parametry filtrace, jen odebereme action a id
    // Pokud přišel _redirect parametr (např. z poptávky), vrátíme se tam
    if (!empty($_GET['_redirect'])) {
        $redirect_url = $_GET['_redirect'];
        // Bezpečnostní kontrola - pouze interní URL
        if (strpos($redirect_url, '/') === 0 || strpos($redirect_url, 'https://www.wellnesstrade.cz') === 0) {
    ...
    $redirect_query = !empty($redirect_params) ? '?' . http_build_query($redirect_params) : '';

    header('location: https://www.wellnesstrade.cz/admin/pages/invoices/zalohove-faktury' . $redirect_query);
    exit;
}
...
<?php
    while ($invoice = mysqli_fetch_array($invoices_query)) {
        ...
        ?>
<tr class="even<?php if (!empty($invoice['storno']) && $invoice['storno'] == 1) { ?> storno-row<?php } ?>">
	<td class="text-center">
        <?php if (!empty($invoice['storno']) && $invoice['storno'] == 1) { ?>
            <span class="label label-danger" style="display: inline-block; margin-bottom: 3px;">STORNO</span><br>
        <?php } ?>
        <strong><a href="https://www.wellnesstrade.cz/admin/data/invoices/demands/Zalohova_faktura_<?= $invoice['id'] ?>.pdf?t=<?= $currentDate->getTimestamp() ?>" target="_blank"><?= $invoice['id'] ?></a></strong><br><small><?= $invoice['dateformated'] ?></small>
    </td>
...
';
	...
	<td style="text-align: center;">
				...
				$current_params['action'] = 'storno_invoice';
				$current_params['id'] = $invoice['id'];
				$storno_url = 'zalohove-faktury?' . http_build_query($current_params);
				?>
				...
				<a href="<?= $storno_url ?>"
				   ...
				   title="Zrušit storno"
				   onclick="return confirm('Opravdu chcete ZRUŠIT storno faktury č. <?= $invoice['id'] ?>?')">
				...
				<a href="<?= $storno_url ?>"
				   ...
				   onclick="return confirm('Opravdu chcete stornovat fakturu č. <?= $invoice['id'] ?>?')">
