...
$pagetitle = "Export zálohových faktur";

if (isset($_REQUEST['action']) && $_REQUEST['action'] == 'imported') {

    $clientquery = $mysqli->query("UPDATE demands_advance_invoices_exports SET imported = '" . $_REQUEST['value'] . "' WHERE id = '" . $_REQUEST['id'] . "'") or die($mysqli->error);

    header('Location: https://www.wellnesstrade.cz/admin/pages/invoices/export-zalohovych-faktur?success=imported');

}

if (isset($_REQUEST['action']) && $_REQUEST['action'] == 'email') {

    $mysqli->query("UPDATE administration_settings SET value = '" . $_POST['generation_email'] . "' WHERE param = 'advance_invoices_export'") or die($mysqli->error);

    header('Location: https://www.wellnesstrade.cz/admin/pages/invoices/export-zalohovych-faktur?success=email');
}
...
<div class="row">
    ...
    <div class="col-md-4 col-sm-5">
        <form method="post" role="form" action="export-zalohovych-faktur?action=email">
            <div class="form-group">
                <label class="col-sm-4 control-label">E-mail pro export zálohovek</label>
    ...
    $ordersmaxquery = $mysqli->query('SELECT COUNT(*) AS NumberOfOrders FROM demands_advance_invoices_exports') or die($mysqli->error);
    ...
    $invoices_query = $mysqli->query('SELECT *, DATE_FORMAT(date, "%Y-%m") as dateformated FROM demands_advance_invoices_exports order by id desc limit ' . $s_pocet . ',' . $perpage) or die($mysqli->error);
?>
...
<?php
	...
	<td class="text-center" <?php if (isset($invoice['imported']) && $invoice['imported'] == 'ano' || $invoice['invoice_number'] == 0) {echo 'style="background-color: #ebebeb;"';}?>><a href="/admin/data/demands_invoices/invoices_<?= $invoice['dateformated'] ?>.xml" target="_blank"><?= $invoice['dateformated'] ?></a></td>
	...
	<td style="text-align: center; <?php if (isset($invoice['imported']) && $invoice['imported'] == 'ano' || $invoice['invoice_number'] == 0) {echo 'background-color: #ebebeb; ';}?>">
		<?php if ($invoice['invoice_number'] > 0) { ?>
            <a class="btn btn-default btn-icon icon-left hidden-print"
               href="../../data/export/invoices_demands/demands_invoices_<?= $invoice['dateformated'] ?>.xml" target="_blank">
                Zobrazit
                <i class="entypo-search"></i>
            </a>

            <a class="btn btn-primary btn-icon icon-left hidden-print"
               download href="../../data/export/invoices_demands/demands_invoices_<?= $invoice['dateformated'] ?>.xml">
                Stáhnout
            ...
            <a class="btn btn-blue btn-icon icon-left hidden-print"
               href="./export-zalohovych-faktur?id=<?= $invoice['id'] ?>&value=ano&action=imported">
                Označit za nahrané
            ...
            <a class="btn btn-green btn-icon icon-left hidden-print"
               href="./export-zalohovych-faktur?id=<?= $invoice['id'] ?>&value=ne&action=imported">
                Již nahrané
                <i class="entypo-check"></i>
            </a>
...
<div class="row">
	<div class="col-md-12">
		<center><ul class="pagination pagination-sm">
			<?php $currentpage = "export-zalohovych-faktur";
