...
if (mysqli_num_rows($hottubquery) > 0) {
    ...
    if (isset($_REQUEST['action']) && $_REQUEST['action'] == "remove_picture") {
        ...
        header('location: https://www.wellnesstrade.cz/admin/pages/warehouse/zobrazit-saunu?id=' . $id . '&remove=success');
        exit;
    }
    ...
    if($hottub['brand'] == 'IQue'){

        $link_secret = 'IQU_bgewKD';

    }elseif($hottub['brand'] == 'Lovia'){

        $link_secret = 'LOV_qJcUBZ';

    }elseif($hottub['brand'] == 'Quantum'){

        $link_secret = 'QUA_jEjsaI';

    }elseif($hottub['brand'] == 'Pergola'){

        $link_secret = 'PER_JoifaE';

    }elseif($hottub['brand'] == 'Espoo'){
    ...
    if (isset($_REQUEST['action']) && $_REQUEST['action'] == "remove_change") {
        ...
        header('location: https://www.wellnesstrade.cz/admin/pages/warehouse/zobrazit-saunu?id=' . $_REQUEST['id']);
    ...
    if (isset($_REQUEST['action']) && $_REQUEST['action'] == "products") {
        ...
        if (substr($value, 0, 9) == "warehouse") {
            ...
            // přiřazení produktu od jiné sauny
        ...
        header('location: https://www.wellnesstrade.cz/admin/pages/warehouse/zobrazit-saunu?id=' . $hottub['id']);
                    ...
                    <div style="width: 90%;float: left;">
                        ...
                        if ($access_edit) { ?>
                            <div style="float:right;margin-top: 0px;">
                                <a href="upravit-saunu?id=<?= $hottub['id'] ?>" class="btn btn-primary btn-sm btn-icon icon-left">
                    ...
                    <div class="row info-list" style="margin: 0 -10px 0 -5px;">
                        ...
                        if ($hottub['demand_id'] != 0) { ?>
                            ...
                            <table style="width: 100%; float: left; margin-top: 10px;">
                                ...
                                if (isset($hottub['customer']) && $hottub['customer'] == 1) {

                                    $i = 0;
                                    $specsquery = $mysqli->query('SELECT * FROM specs WHERE product = 1 AND warehouse_spec = 1 order by rank asc') or die($mysqli->error);
                                    while ($specs = mysqli_fetch_assoc($specsquery)) {
                                        ...
                                        <td style="background-color: <?= $specs['bg_colour'] ?>;  color: #000; width: 14%; padding: 3px 5px; border-bottom: 1px solid #fff; border-right: 4px solid #fff;text-align: center;"><?php if (!empty($params['value'])) { echo $params['value']; } else { echo '-'; } ?> <?php
                                            ...
                                            if (!empty($demandsspecs['value']) && !empty($params['value']) && $demandsspecs['value'] != $params['value'] && $specs['is_demand'] == 1) { ?><i style="color: #d42020;font-size: 16px; margin-left: 1px;margin-top: -3px;position: absolute;" class="entypo-attention" data-toggle="tooltip" data-placement="top" title="" data-original-title="Specifikace u sauny neodpovídá zvolené specifikaci u poptávky."></i><?php } ?></td>
    ...
    GROUP BY s.id order by s.demand_category asc, s.name asc") or die($mysqli->error);
        ...
        <div class="col-sm-6">
            <div class="panel panel-primary" data-collapsed="0">
                ...
                <div class="panel-heading">
                    <div class="panel-title">
                        <?php if (mysqli_num_rows($specsquery) > 0) { ?><i style="color: #d42020;" class="entypo-attention" data-toggle="tooltip" data-placement="top" title="" data-original-title="Specifikace u sauny neodpovídá zvolené specifikaci u poptávky."></i><?php } else { ?><i style=" color: #00a651;" class="entypo-check"></i><?php } ?> Nutné změny pro poptávku
                    </div>
                </div>
            ...
            function current_changes($changes)
            {
                ...
                <div class="col-sm-12" style="margin-bottom: 6px; padding: 0; color: #000000; padding-left: 22px; float: left;">
                    ...
                    if ($single['type'] == 'warehouse') { ?>
                        ...
                        <a href="/admin/pages/warehouse/zobrazit-saunu?id=<?= $warehouse_hottub['id'] ?>" target="_blank" style="color: #ff5722; font-weight: bold;">- Sauna #<?= $warehouse_hottub['serial_number'] ?> - <?= $warehouse_hottub['name'] ?></a>

                        <?php

                        $task_content .= ' - Sauna #' . $warehouse_hottub['serial_number'] . ' - ' . $warehouse_hottub['name'];

                    }?>

                    <div style="float: right;">
                        <a href="zobrazit-saunu?id=<?= $hottub['id'] ?>&requester_id=<?= $changes['warehouse_id'] ?>&action=done&spec_id=<?= $changes['specification_id'] ?>" class="btn btn-green"  style="margin-right: 8px;" <?php if ($changes['type'] == 'missing') {echo 'disabled';}?>>
                            <i class="entypo-check"></i>
                        </a>
                        <span style=" border-right: 1px solid #cccccc;"></span>
                        <a href="zobrazit-saunu?id=<?= $hottub['id'] ?>&requester_id=<?= $changes['warehouse_id'] ?>&action=remove_change&spec_id=<?= $changes['specification_id'] ?>" class="btn btn-default" style="margin-left: 11px;">
                            <i class="entypo-trash"></i>
                        </a>
                    </div>
...
';
            ...
            <div class="panel panel-primary" data-collapsed="0">
                ...
                <div class="panel-heading">
                    <div class="panel-title">
                        Budoucí plánované změny u sauny
                    </div>
                ...
                <div class="availabilities_<?= $specs['id'] ?>_<?= $params['id'] ?>" style="display: none;">
                    ...
                    <div class="col-sm-6">
                        ...
                        if(!empty($product)){
                            ...
                            <form role="form" method="post" name="myform" id="demand_form" class="form-horizontal form-groups-bordered" enctype="multipart/form-data" action="zobrazit-saunu?id=<?= $hottub['id'] ?>&action=products&d_product_id=<?= $product['dem_id'] ?>">
                                ...
                                if (mysqli_num_rows($get_products) > 0) {
                                    ...
                                    <select name="value" class="form-control">
                                        ...
                                        while ($warehouse_product = mysqli_fetch_array($warehouse_products_query)) {
                                            ...
                                            <option value="ware-product-<?= $warehouse_product['id'] ?>">Skladová sauna #<?= $warehouse_product['serial_number'] ?> -  <?= $warehouse_product['name'] ?></option>
            ...
            <div class="panel panel-primary" data-collapsed="0">
                ...
                <div class="panel-heading">
                    <div class="panel-title">
                        Naplánování fyzické úpravy sauny
                    </div>
                ...
                <div class="panel-body">
                    ...
                    <script type="text/javascript">
                        jQuery(document).ready(function($)
                        {
                            ...
                            $('.remove-picture').click(function() {
                                ...
                                $.get("./zobrazit-saunu?id=<?= $id ?>&action=remove_picture&picture="+picture);
                    ...
                    while ($specs = mysqli_fetch_array($specs_query)) {
                        ...
                        // VŠECHNY VALUES U PROVEDENÍ SAUNY
                ...
                <div class="panel-body">
                    ...
                    if (mysqli_num_rows($task_query) > 0) {
                        ...
                        while($task = mysqli_fetch_array($task_query)){
                            ...
                            task($task, $client['avatar'], $access_edit, 'pages/warehouse/zobrazit-saunu?id=' . $hottub['id']);
                ...
                <div class="panel-body">
                    ...
                    if(!empty($task_content)){
                        ...
                        <div class="well" style="width: 100%; float:inherit; margin: 0 auto 50px;">
                            <form autocomplete="off" id="taskform" role="form" method="post" enctype='multipart/form-data' action="/admin/controllers/task-controller?task=add&redirect=pages/warehouse/zobrazit-saunu&redirectid=<?= $hottub['id'] ?>">

                                <div class="form-group" style="float:left; width: 100%;">
                                    <input type="text" style="width: 64%; float: left; margin-right: 2%; margin-bottom: 8px;" name="title" placeholder="Název úkolu" class="form-control" id="field-1" value="Úprava sauny <?= $name ?>" required>
