...
//
        ...
        if (isset($_REQUEST['redirect']) && $_REQUEST['redirect'] != "") {
            ...
            Header("Location:https://www.wellnesstrade.cz/admin/pages/warehouse/zobrazit-pergolu?id=" . $_REQUEST['id'] . "&success=edit");
...
<form role="form" method="post" name="myform" class="form-horizontal form-groups-bordered validate" action="upravit-pergolu?id=<?= $_REQUEST['id'] ?>&action=edit<?php if (isset($_REQUEST['redirect']) && $_REQUEST['redirect'] != "") {echo '&redirect=' . $_REQUEST['redirect'];}?>">
<input type="hidden" name="length" value="14">
	...
	$demandsq = $mysqli->query("SELECT user_name, id FROM demands WHERE customer = 4 
...
?><option value="" selected></option>
		...
		<div class="col-md-6">
			...
			<div class="panel panel-primary" data-collapsed="0">
				...
				<div class="panel-heading">
					...
					<label class="col-sm-3 control-label">Pergoly</label>
    ...
    $virivky_query_new = $mysqli->query("SELECT id, connect_name, fullname, brand FROM warehouse_products WHERE customer = 4 ORDER BY brand");
...
<?php while ($typ = mysqli_fetch_array($virivky_typy)) { ?>
						...
						<div class="col-sm-7">
                            <div class="radio" style="width: 140px; float: left; text-align: left;">
                                <label>
                                    <input 
                                            type="text"
                                            name="<?= $hottub['product'] ?>_<?= $typ['seo_url'] ?>_<?= $specs['seoslug'] ?>"
  ...
  <a href="./pergoly?id=<?= $_REQUEST['id'] ?>"><button type="button" class="btn btn-primary">Zpět</button></a>
