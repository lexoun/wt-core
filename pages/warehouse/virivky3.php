...
$type = isset($_REQUEST['type']) ? $mysqli->real_escape_string($_REQUEST['type']) : null;
$customer = isset($_REQUEST['customer']) ? $mysqli->real_escape_string($_REQUEST['customer']) : null;
...
// Nové specifikace
if (isset($_REQUEST['spec5'])) { $spec5 = $_REQUEST['spec5']; }
if (isset($_REQUEST['spec1'])) { $spec1 = $_REQUEST['spec1']; }
if (isset($_REQUEST['spec2'])) { $spec2 = $_REQUEST['spec2']; }
...
// Zpracování nových filtrů pro specifikace
if (isset($spec5) && $spec5 != '') {
    $queryBuilder['spec5'] = $spec5;
    $sqlQueryBuilder[] .= "w.id IN (SELECT client_id FROM warehouse_specs_bridge WHERE specs_id = 5 AND value = '" . $mysqli->real_escape_string($spec5) . "') ";
}
if (isset($spec1) && $spec1 != '') {
    $queryBuilder['spec1'] = $spec1;
    $sqlQueryBuilder[] .= "w.id IN (SELECT client_id FROM warehouse_specs_bridge WHERE specs_id = 1 AND value = '" . $mysqli->real_escape_string($spec1) . "') ";
}
if (isset($spec2) && $spec2 != '') {
    $queryBuilder['spec2'] = $spec2;
    $sqlQueryBuilder[] .= "w.id IN (SELECT client_id FROM warehouse_specs_bridge WHERE specs_id = 2 AND value = '" . $mysqli->real_escape_string($spec2) . "') ";
}
...
if (isset($search) && $search != "") { ?>
                ...
                <div class="row">
                    <div class="col-md-12" style="text-align: left;">
                        ...
                        <div class="btn-group">
                            ...
                            $brands_query = $mysqli->query("SELECT brand FROM warehouse_products WHERE active = 'yes' AND brand != '' AND customer = 1 GROUP BY brand");
                        ...
                        <div class="btn-group" style="padding-top: 0px;">
                            ...
                            <a href="virivky?special=3" style="padding: 5px 11px !important;" class="btn <?php if(!empty($_REQUEST['special']) && $_REQUEST['special'] == 3){ echo 'btn-primary'; }else{ echo 'btn-white'; } ?>">
                                Vše s MicroSilkem</a>
                            <a href="virivky?special=8" style="padding: 5px 11px !important;" class="btn <?php if(!empty($_REQUEST['special']) && $_REQUEST['special'] == 8){ echo 'btn-primary'; }else{ echo 'btn-white'; } ?>">
                                Vše s InClearem</a>
                        </div>
    ...
    $max_query = $mysqli->query("SELECT w.id as id FROM warehouse w, warehouse_products p WHERE w.product = p.connect_name AND w.customer = 1 AND demand_id = 0 AND w.reserved_showroom = 0 AND w.status != 7 $this_query") or die($mysqli->error);
    $max = mysqli_num_rows($max_query);
...
//    $this_query = preg_replace("/AND w.reserved_showroom <> '0'/", '$2', $query);
    ...
    $max_query = $mysqli->query("SELECT w.id as id FROM warehouse w, warehouse_products p WHERE w.product = p.connect_name AND w.customer = 1 AND w.reserved_showroom <> '0' $this_query") or die($mysqli->error);
    $max = mysqli_num_rows($max_query);
    ...
    $max_query = $mysqli->query("SELECT w.id as id FROM warehouse w, warehouse_products p WHERE w.product = p.connect_name AND w.customer = 1 AND demand_id > 0 AND w.reserved_showroom = 0 $this_query") or die($mysqli->error);
    $max = mysqli_num_rows($max_query);

    ?>

                            <a href="<?= $thisPage . $pageSymbol . 'customer=1' ?>"
                                style="padding: 5px 11px !important;"
                                class="btn <?php if (isset($customer) && $customer == 1) {echo 'btn-primary';} else {echo 'btn-white';} ?>">
                                Prodané (<?= $max ?>)
                                </label></a>
                        </div>
                    </div>
    ...
    $hottubs_products_query = $mysqli->query("SELECT * FROM warehouse_products WHERE active = 'yes' AND customer = 1 ORDER BY rank");
    ...
    ?>
                        ...
                        <div class="btn-group" style="margin-bottom: 5px;">
                            <a href="<?php echo preg_replace('/&?spec5=[^&]*/', '', $currentpage); ?>" style="padding: 5px 11px !important; margin-top:4px;" class="btn <?php echo (!isset($spec5)) ? 'btn-primary' : 'btn-white'; ?>">
                                Všechna provedení
                            </a>
                            <?php
                            $spec5_query = $mysqli->query("SELECT value FROM warehouse_specs_bridge WHERE specs_id = 5 AND value != '' AND value != '-' GROUP BY value ORDER BY value ASC");
                            while ($s5 = mysqli_fetch_array($spec5_query)) {
                                $button_data = [
                                    'param' => isset($spec5) ? $spec5 : '',
                                    'specs_id' => 5,
                                    'request_name' => 'spec5',
                                    'url' => 'spec5=' . urlencode($s5['value']),
                                    'name' => ucfirst($s5['value']),
                                    'current_param' => $s5['value']
                                ];
                                echo spec_button($button_data);
                            }
                            ?>
                        </div>
                        <br>
                        ...
                        <div class="btn-group" style="margin-bottom: 5px;">
                            <a href="<?php echo preg_replace('/&?spec1=[^&]*/', '', $currentpage); ?>" style="padding: 5px 11px !important; margin-top:4px;" class="btn <?php echo (!isset($spec1)) ? 'btn-primary' : 'btn-white'; ?>">
                                Všechny akryly
                            </a>
                            <?php
                            $spec1_query = $mysqli->query("SELECT value FROM warehouse_specs_bridge WHERE specs_id = 1 AND value != '' AND value != '-' GROUP BY value ORDER BY value ASC");
                            while ($s1 = mysqli_fetch_array($spec1_query)) {
                                $button_data = [
                                    'param' => isset($spec1) ? $spec1 : '',
                                    'specs_id' => 1,
                                    'request_name' => 'spec1',
                                    'url' => 'spec1=' . urlencode($s1['value']),
                                    'name' => ucfirst($s1['value']),
                                    'current_param' => $s1['value']
                                ];
                                echo spec_button($button_data);
                            }
                            ?>
                        ...
                        <div class="btn-group">
                            <a href="<?php echo preg_replace('/&?spec2=[^&]*/', '', $currentpage); ?>" style="padding: 5px 11px !important; margin-top:4px;" class="btn <?php echo (!isset($spec2)) ? 'btn-primary' : 'btn-white'; ?>">
                                Všechna opláštění
                            </a>
                            <?php
                            $spec2_query = $mysqli->query("SELECT value FROM warehouse_specs_bridge WHERE specs_id = 2 AND value != '' AND value != '-' GROUP BY value ORDER BY value ASC");
                            while ($s2 = mysqli_fetch_array($spec2_query)) {
                                $button_data = [
                                    'param' => isset($spec2) ? $spec2 : '',
                                    'specs_id' => 2,
                                    'request_name' => 'spec2',
                                    'url' => 'spec2=' . urlencode($s2['value']),
                                    'name' => ucfirst($s2['value']),
                                    'current_param' => $s2['value']
                                ];
                                echo spec_button($button_data);
                            }
                            ?>
