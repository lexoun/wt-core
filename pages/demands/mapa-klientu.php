...
$pagetitle = "Mapa klientů";

$query = "";
$currentpage = "mapa-klientu";
$allow_sites = "";
...
if (isset($customer) && $customer == 0) {
    ...
    if (isset($_REQUEST['start_date']) || isset($_REQUEST['end_date'])) {
        ...
        $second_product_query = $mysqli->query("SELECT * FROM demands WHERE status = '5' $query_double order by id asc") or die($mysqli->error);
...
<!-- Pager for search results --><div class="col-md-12 well" style="border-color: #ebebeb; margin-top: 8px;background-color: #fbfbfb;">
    ...
    <form role="form" method="get" class="form-horizontal form-groups-bordered validate" action="mapa-klientu" enctype="multipart/form-data" novalidate="novalidate">
...
if (isset($second_product_query)) {
    while ($allclients = mysqli_fetch_array($second_product_query)) {
        ...
        if (isset($_REQUEST['start_date']) || isset($_REQUEST['end_date'])) {
            ...
            $second_date_query = $mysqli->query("SELECT * FROM demands WHERE status = '5' AND id = '" . $allclients['demand_id'] . "' order by id asc") or die($mysqli->error);

            $allclients = mysqli_fetch_array($second_date_query);

            if ($allclients['status'] != 5) {
