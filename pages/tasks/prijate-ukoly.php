...
<div id="taskform" class="well" style="display: none; width: 700px; float:inherit; margin: 0 auto 50px;">
    <h2 class="specialborderbottom" style="margin-bottom: 20px;">Nový úkol</h2>
		<form class="validate" id="taskform" role="form" method="post" enctype='multipart/form-data' action="/admin/controllers/task-controller?task=add&redirect=pages/tasks/prijate-ukoly">
...
<div class="row">
  <div class="col-md-5">
    <div class="btn-group" style="text-align: left;">
            <?php $mark = "?";?>
            <a href="prijate-ukoly"><label class="btn btn-lg <?php if (!isset($type)) { ?>btn-primary<?php } else { ?>btn-white<?php } ?>">
              Vše
            </label></a>

            <a href="?type=demand"><label class="btn btn-lg <?php if ($type == 'demand' && isset($type)) { ?>btn-primary<?php } else { ?>btn-white<?php } ?>">
              Poptávky
            </label></a>
            <a href="?type=client"><label class="btn btn-lg <?php if ($type == 'client' && isset($type)) { ?>btn-primary<?php } else { ?>btn-white<?php } ?>">
              Klienti
            </label></a>
            <a href="?type=general"><label class="btn btn-lg <?php if ($type == 'general') { ?>btn-primary<?php } else { ?>btn-white<?php } ?>">
              Obecné
            </label></a>
...
$currentpage = 'prijate-ukoly' . $string;

$tasks_max_query = $mysqli->query("SELECT COUNT(*) AS MaxNumber FROM mails_recievers tgts, tasks tsks WHERE tgts.admin_id = '" . $client['id'] . "' AND tsks.id = tgts.type_id AND tgts.type = 'task' AND tsks.due < CURDATE() $query") or die($mysqli->error);
$tasks_max = mysqli_fetch_array($tasks_max_query);
$pocet_prispevku = $tasks_max['MaxNumber'];
$demandstasksquery = $mysqli->query("SELECT *, DATE_FORMAT(date, '%d. %m. %Y') as dateformated, DATE_FORMAT(due, '%d. %m. %Y') as dueformated FROM mails_recievers tgts, tasks tsks WHERE tgts.admin_id = '" . $client['id'] . "' AND tsks.id = tgts.type_id AND tgts.type = 'task' AND tsks.due < CURDATE() $query ORDER BY tsks.id DESC limit " . $s_pocet . "," . $perpage) or die($mysqli->error);

while ($demandstasks = mysqli_fetch_array($demandstasksquery)) {
    task($demandstasks, $client['avatar'], $access_edit, 'pages/tasks/prijate-ukoly');
}?>
