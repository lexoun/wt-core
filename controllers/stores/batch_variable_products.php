...
while ($shop = mysqli_fetch_array($shops_query)) {
    ...
    if (mysqli_num_rows($importer_query) > 0) {
        ...
        while ($product = mysqli_fetch_array($importer_query)) {
            ...
            $print = $print . '#' . $i . '-' . $shop['slug'] . ' - Sys ID: ' . $product['product_id'] . ' --- Shop ID: ' . $product['site_id'] . ' --- Delivery: ' . $delivery . ' --- Instock: ' . $product['instock'] . '<br>';
            ...
            $variations_query = $mysqli->query("SELECT v.id as variation_id, s.site_id, SUM(stock.instock) as instock FROM products_variations v, products_variations_sites s, products_stocks stock WHERE s.site_id <> 0 AND v.product_id = '" . $product['id'] . "' AND s.variation_id = v.id AND s.site = '" . $shop['slug'] . "' AND stock.variation_id = v.id GROUP BY v.id") or die($mysqli->error);
            ...
            while ($variation = mysqli_fetch_array($variations_query)) {
                ...
                $current_variation = array(
                    ...
                    "manage_stock" => true,
                    "stock_quantity" => $variation['instock'],
                    "in_stock" => true,
                    "backorders" => "notify",
                    "purchaseable" => true,
                ...
                array_push($all_variations, $current_variation);

            }

            $data_vari = ['update' => $all_variations];

            try {

                $woocommerce_connection->post('products/' . $product['site_id'] . '/variations/batch', $data_vari);
            ...
            $mysqli->query("DELETE FROM cron_jobs WHERE product_id = '" . $product['product_id'] . "' AND type = 'batch_variable' AND shop_slug = '" . $shop['slug'] . "'") or die($mysqli->error);
