...
if(!empty($_REQUEST['invoice_id'])){ $invoice_id = $_REQUEST['invoice_id']; }


$invoice_query = $mysqli->query("SELECT * FROM orders_invoices WHERE id = '$invoice_id' order by id desc");

if(mysqli_num_rows($invoice_query) != 1){ echo 'error2'; exit; }
...
} elseif($invoice['status'] == 'odd') {

    $prev_id = $invoice['invoice_id'];
...
if ($invoice['type'] == 'order') {
    ...
    $order_query = $mysqli->query("SELECT *, DATE_FORMAT(order_date, '%d. %m. %Y') as dateformated FROM orders WHERE id = '".$invoice['order_id']."'");
    $order = mysqli_fetch_array($order_query);

    $products_query = $mysqli->query("SELECT *, p.id as ajdee, o.price as price 
        FROM orders_products_bridge o 
            LEFT JOIN products p ON p.id = o.product_id 
        WHERE o.aggregate_id = '".$invoice['order_id']."' AND o.aggregate_type = 'order'");
    ...
    $redirect = "https://www.wellnesstrade.cz/admin/pages/orders/zobrazit-objednavku?id=" . $order['id'] . "&success=generate_invoice";

} elseif ($invoice['type'] == 'service') {

    $order_query = $mysqli->query("SELECT *, DATE_FORMAT(date, '%d. %m. %Y') as dateformated FROM services WHERE id = '".$invoice['order_id']."'");
    $order = mysqli_fetch_array($order_query);

    $products_query = $mysqli->query("SELECT *, p.id as ajdee, b.price as price, b.discount_net FROM products p,  services_products_bridge b WHERE p.id = b.product_id AND b.aggregate_id = '".$invoice['order_id']."'");
    ...
    $redirect = "https://www.wellnesstrade.cz/admin/pages/services/zobrazit-servis?id=" . $order['id'] . "&success=generate_invoice";
...
$insert_date = date("Y-m-d H:i:s", strtotime($invoice['date']));
$invoice_datetime = date("d. m. Y H:i:s", strtotime($invoice['date']));
...
} elseif (isset($address['billing_country']) && $address['billing_country'] == 'UK') {
    $order_country = 'Velká Británie';
} else {
...
if (isset($order['payment_method']) && $order['payment_method'] == 'cash' || $order['payment_method'] == 'agmobindercardall') {
    ...
    $date_due = date('d. m. Y', strtotime($invoice['date'] . ' + 14 days'));
...
if ($invoice['type'] == 'service') {
    ...
    $items_query = $mysqli->query("SELECT * FROM services_items WHERE service_id = '".$invoice['order_id']."'")or die($mysqli->error);
...
if($invoice['type'] == 'order'){
    ...
    $get_price = $mysqli->query("SELECT SUM(total) as total, SUM(purchase_price) as purchase_price, SUM(discount_net) as discount_net 
        FROM (
            ...
            WHERE o.id = b.aggregate_id AND p.id = b.product_id AND o.id = '".$invoice['order_id']."' AND p.type = 'simple' AND b.aggregate_type = 'order' 
        UNION ALL 
            SELECT sum(b.quantity * b.price) as total, sum(b.purchase_price * b.quantity) as purchase_price, sum(b.discount_net * b.quantity) as discount_net 
            from products_variations v, orders_products_bridge b, orders o 
            WHERE o.id = b.aggregate_id AND v.id = b.variation_id AND o.id = '".$invoice['order_id']."' AND b.variation_id != 0 AND b.aggregate_type = 'order') 
...
<li>FIK: '.$invoice['fik'].'</li>
<li>BKP: '.$invoice['bkp'].'</li>
...
<td style="padding-left: 0;"><strong style="color: #CE1212; font-weight: bold;">'.$currency['bank_account'].'</strong></td>
</tr>
<tr>
...
$mysqli->query("UPDATE orders_invoices SET total_price = '".$total['single']."', price_without_vat = '".$total['without_vat']."', total_vat = '".$total['vat']."', rounded = '".$total['rounded']."' WHERE id = '".$invoice_id."'")or die($mysqli->error);
