...
if (isset($_REQUEST['secretcode']) && $_REQUEST['secretcode'] == "lYspnYd2mYTJm6") {
    ...
    if (mysqli_num_rows($invoices_query) > 0) {
        ...
        while ($invoices = mysqli_fetch_array($invoices_query)) {
            ...
            */
            ...
            if($invoices['currency'] != 'CZK'){ $listOfPrices .= ' - '.$invoices['currency']; }

            if($invoices['currency'] != 'CZK'){ $listOfPrices .= ' - <strong>! '.$invoices['currency'].' !</strong>'; }

            if(!empty($address_invoice['billing_ico'])){ $listOfPrices .= ' - ICO: '.$address_invoice['billing_ico']; }
            if(!empty($address_invoice['billing_dic'])){ $listOfPrices .= ' - DIC: '.$address_invoice['billing_dic']; }
            if($address_invoice['billing_country'] != 'czech'){ $listOfPrices .= ' - COUNTRY : '.$address_invoice['billing_country']; }
        ...
        /* get the xml printed */
        $domtree->save($_SERVER['DOCUMENT_ROOT'] . '/admin/data/export/invoices_demands/demands_invoices_' . $id . '.xml');
          ...
          <tbody>
            ...
            <tr>
             ...
             <tr>
              ...
              <td style="font-size: 13px; line-height: 20px;">https://www.wellnesstrade.cz/admin/data/export/invoices_demands/demands_invoices_' . $id . '.xml</td>
            </tr>
    ...
    */
    ...
    echo '<a href="https://www.wellnesstrade.cz/admin/data/export/invoices_demands/demands_invoices_' . $id . '.xml" target="_blank">https://www.wellnesstrade.cz/admin/data/export/invoices_demands/demands_invoices_' . $id . '.xml</a>';
