...
if (isset($_REQUEST['secretcode']) && $_REQUEST['secretcode'] == "lYspnYd2mYTJm6") {
    ...
    $date = date('Y-m-00', strtotime("-1 months"));
    ...
    $invoices_query = $mysqli->query("SELECT *, i.id as id, DATE_FORMAT(i.date, '%Y-%m-%d') as date, DATE_FORMAT(i.date, '%Y-%m-%dT%H:%i:%s') as datetime FROM orders_invoices i 
...
while($invaice = mysqli_fetch_array($invoices_query)){
...
echo $invaice['id'].'<br>';
    ...
    if (mysqli_num_rows($invoices_query) > 0) {
        ...
        while ($invoices = mysqli_fetch_array($invoices_query)) {
            ...
            $update_query = $mysqli->query("UPDATE orders_invoices SET export_id = '$id' WHERE id = '" . $invoices['id'] . "'") or die($mysqli->error);

        }

        /* get the xml printed */
        $domtree->save($_SERVER['DOCUMENT_ROOT'] . '/admin/data/export/invoices_orders/invoices_' . $id . '.xml');

    }

    $find_query = $mysqli->query("SELECT * FROM orders_invoices_exports WHERE date = '$date'") or die($mysqli->error);

    if (mysqli_num_rows($find_query) > 0) {

        $insert_query = $mysqli->query("UPDATE orders_invoices_exports SET invoice_number = '$count' WHERE date = '$date'") or die($mysqli->error);

    } else {

        $insert_query = $mysqli->query("INSERT INTO orders_invoices_exports (date, invoice_number) VALUES ('$date', '$count')") or die($mysqli->error);

    }

    echo $listOfPrices;

    $subject = 'Balíček objednávek - ' . $id;

    $title = 'Balíček objednávek - ' . $id;
          ...
          <tbody>
            ...
            <tr>
             ...
             <tr>
              ...
              <td style="font-size: 13px; line-height: 20px;">https://www.wellnesstrade.cz/admin/data/export/invoices_orders/invoices_' . $id . '.xml</td>
            </tr>
    ...
    */
    ...
    echo '<a href="https://www.wellnesstrade.cz/admin/data/export/invoices_orders/invoices_' . $id . '.xml" target="_blank">https://www.wellnesstrade.cz/admin/data/export/invoices_orders/invoices_' . $id . '.xml</a>';
