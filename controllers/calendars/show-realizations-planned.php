<?php
if ($_GET['showType'] == 'planned' || $_GET['showType'] == 'all') {
...
function userName($name)
{
    ...
    if (isset($name['billing_name']) && ($name['billing_name'] != '' || $name['billing_surname'] != '')) {
...
if ($show == 'all') {
    ...
    WHERE r.admin_id = tech.id AND tech.role = 'technician' AND b.id = d.billing_id AND d.realization != '0000-00-00' AND d.confirmed = '0' AND d.realization > DATE_SUB(NOW(), INTERVAL 12 MONTH) AND r.type_id = d.id AND (r.type = 'realization_hottub' or r.type = 'realization_sauna') $recieverType GROUP BY d.id") or die($mysqli->error);
